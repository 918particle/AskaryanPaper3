...
\textit{As the reviewer notes, we find $f_0$ after analyzing pulses that pass through an RF dipole transmitter, ice, then an RF dipole receiver.  The resonance frequency $f_0$ of a dipole cannot change, no matter how many are in the RF change, so $f_0$ should be measured like a constant.  However, this is also true of $\gamma$.  The reviewer rightly identifies the application of a double convolution to model this situation.  However, the convolution of two decaying exponentials with the same decay rate produces a third exponential with the same decay rate, multiplied by time.  Thus, $\gamma$ is measured correctly.  Further, our result for the error estimate of $\gamma$ from calibration pulse events is not larger than the mean.  To clarify these points, we have modified the first paragraph in Sec. IV.  Regarding the paragraph beginning with ``Several points,'' we also have noted that fitting $f_0$ and $\gamma$ as free parameters against the UHECR data allows the results for $f_0$ and $\gamma$ to be affected by $\sigma_t$ (which does not have to be constant) and reflected pulses.  These effects are not present in the calibration pulser events.}
